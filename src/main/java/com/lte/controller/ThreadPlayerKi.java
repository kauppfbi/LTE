 ...
 *
 */
public class ThreadPlayerKi extends Thread {
	 ...
	 * @param connection
	 */
	public ThreadPlayerKi(ControllerPlayerKi controller3, GameInfo gameInfo, Settings settings,
			AlgorithmManager algorithmManager, DBconnection connection) {
	 ...
	 * 
	 * @param nextMove
	 *            next move given by the listener
	 * @throws Exception
	...
	public synchronized void run() {
		...
		while (true) {
			...
			if (nextMove == -1) {
				...
				try {
					move = algorithmManager.ParallelAlphaBeta(currentGameScore.getField(), 10,
		...
		if (gameInfo.getOwnPoints() == 3) {
			connection.updateScoreOfGame(gameInfo.getGameID(), gameInfo.getOwnPoints(), gameInfo.getOpponentPoints(),
					"X");
		}
