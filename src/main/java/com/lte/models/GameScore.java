...
/**
 * This Class represents the the score data in a model.<br>
 * @author Fabian S.
 *
 */
public class GameScore {
	...
	// ************Initialisiere*************************************************
	/**
	 * Initializes all slots of the corresponding field of the game score with
	 * the initial character 0
	 */
	 ...
	 * 
	 * @param arraycolumn
	 *            column for the move on the field
	 * @param playerCharacter
	 *            character for the given move. 2 or 1.
	...
	public int[][] winWhere() {
		...
		for (int column = 0; column < 4; column++) {
			for (int row = 0; row < 6; row++) {
				int rating = 0;
				rating = evalFunction(field[column][row], field[column + 1][row], field[column + 2][row],
						field[column + 3][row]);
		...
		for (int column = 0; column < 7; column++) {
			for (int row = 0; row < 3; row++) {
				int rating = 0;
				rating = evalFunction(field[column][row], field[column][row + 1], field[column][row + 2],
						field[column][row + 3]);
				if (rating == 100000 || rating == -100000) {
					winWhere[0][0] = column;
					winWhere[0][1] = row;
		...
		for (int column = 0; column < 4; column++) {
			for (int row = 0; row < 3; row++) {
				int rating = 0;
				rating = evalFunction(field[column][row], field[column + 1][row + 1], field[column + 2][row + 2],
						field[column + 3][row + 3]);
				if (rating == 100000 || rating == -100000) {
					winWhere[0][0] = column;
					winWhere[0][1] = row;
					winWhere[1][0] = column + 1;
	 ...
	 * 
	 * @return char 'N' for no winner. 1 for the own player and 2 for the
	 *         opponent.
	 */
	public byte isWon() {
		byte isGewonnen = 0;
		...
		for (byte column = 0; column < 4; column++) {
			for (byte row = 0; row < 6; row++) {
				...
				if (field[column][row] == 1 && field[column + 1][row] == 1 && field[column + 2][row] == 1
						&& field[column + 3][row] == 1) {
					return 1;
				}
				if (field[column][row] == 2 && field[column + 1][row] == 2 && field[column + 2][row] == 2
						&& field[column + 3][row] == 2) {
					return 2;
				}
			}
		}
		...
		for (byte column = 0; column < 7; column++) {
			for (byte row = 0; row < 3; row++) {
				...
				if (field[column][row] == 1 && field[column][row + 1] == 1 && field[column][row + 2] == 1
						&& field[column][row + 3] == 1) {
					return 1;
				}
				if (field[column][row] == 2 && field[column][row + 1] == 2 && field[column][row + 2] == 2
						&& field[column][row + 3] == 2) {
					return 2;
				}
			}
		}
		...
		for (byte column = 0; column < 4; column++) {
			for (byte row = 0; row < 3; row++) {
				...
				if (field[column][row] == 1 && field[column + 1][row + 1] == 1 && field[column + 2][row + 2] == 1
						&& field[column + 3][row + 3] == 1) {
					return 1;
				}
				if (field[column][row] == 2 && field[column + 1][row + 1] == 2 && field[column + 2][row + 2] == 2
						&& field[column + 3][row + 3] == 2) {
					return 2;
				}
			}
		}
	...
	public int eval() {
		...
		for (byte column = 0; column < 4; column++) {
			for (byte row = 0; row < 6; row++) {
				...
				ratingOverall = ratingOverall + evalFunction(field[column][row], field[column + 1][row],
		...
		for (byte column = 0; column < 7; column++) {
			for (byte row = 0; row < 3; row++) {
				...
				ratingOverall = ratingOverall + evalFunction(field[column][row], field[column][row + 1],
		...
		for (byte column = 0; column < 4; column++) {
			for (byte row = 0; row < 3; row++) {
				...
				ratingOverall = ratingOverall + evalFunction(field[column][row], field[column + 1][row + 1],
